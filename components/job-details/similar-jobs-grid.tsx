      ...
      <section className={className}>
        ...
        <div className="grid md:grid-cols-3 gap-4">
          {[...Array(limit)].map((_, i) => (
            <div
              key={i}
              className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg p-4 h-[120px] animate-pulse"
            >
        ...
        {jobs.map((job) => (
          <Link key={job.id} href={`/jobs/${job.id}`} className="group">
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg p-4 h-[120px] flex flex-col transition-all hover:shadow-md hover:border-gray-300 dark:hover:border-gray-700">
              {/* Company */}
              <div className="flex items-center gap-3 mb-2">
                <div className="w-8 h-8 rounded-md overflow-hidden bg-gradient-to-br from-gray-200 to-gray-300 dark:from-gray-700 dark:to-gray-600 flex items-center justify-center flex-shrink-0">
              ...
              {/* Title */}
              <h3 className="font-semibold text-gray-900 dark:text-white truncate mb-auto group-hover:text-blue-600 dark:group-hover:text-blue-400">
                {job.title}
              </h3>

              {/* Meta */}
              <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                <span>{formatSalary(job.salaryUSD)}/mo</span>
